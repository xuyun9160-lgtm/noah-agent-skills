...
|-----------|-------------|-------|-----------|
| OpenClaw | Download ZIP, extract, and copy packaged skills into `~/.openclaw/skills/` | Global / workspace | < 2 min |
| Claude Code CLI | Copy all packaged skills under `search-skills/` into `~/.claude/skills/` | Global (all projects) | 2 min |
| Cursor / VS Code / JetBrains (with Claude Extension) | Reuse all packaged skills from `~/.claude/skills/` | Global (all projects) | 2 min |
| Cursor (Built-in AI) | Copy packaged rule-capable skill instructions from `search-skills/` into `~/.cursor/rules/` | Global (all projects) | 2-3 min |
...
<details>
<summary><b>OpenClaw</b> - Copy skills directly</summary>
...
If you still want a local config file, you can optionally create:

```bash
mkdir -p ~/.openclaw/.secrets
cp noah-market.env.example ~/.openclaw/.secrets/noah-market.env
```
...
Copy packaged rule-capable skill instructions into the Cursor rules directory.
